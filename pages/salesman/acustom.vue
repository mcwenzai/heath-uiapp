	...
	<view class="cus-list">
		...
		<view class="container">
			...
			<view class="content ">
				...
				<view v-if="isData">
					<view class="co" v-for="(item,index) in customerList" :key="item.id" @click="goHelp(item)">
						<view class="name">
							<view class="left">
								<text>{{item.name}}</text>
							</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			// 去代客下单
			goHelp(e) {
				if (e.buyerQualificationStatus == 0) {
					uni.setStorage({
						key: 'customerId',
						data: e.id
					})
					getApp().globalData.customerData = e
					uni.switchTab({
						url: '../../pages/index/index'
					})
				} else {
					this.$refs.uToast.show({
						title: '用户认证信息异常，无法代客下单',
						type: 'error'
					})
				}
			}
