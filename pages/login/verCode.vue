	...
	methods: {
		...
		getCode2() {
			if (this.status == false) {
				...
				const url =
					"/user/appUser/sendLoginSms" +
					setRequestVerification(data, 0);
				this.$myRequest({
					method: "GET",
					url: url,
					data: data,
		...
		login(param) {
			...
			const data = {
				...
				pushJson: { ...this.deviceInfo, type: type },
			};
			const url =
				"/user/appUser/phoneLogin" + setRequestVerification(data, 1);
			this.$myRequest({
				method: "GET",
				url: url,
				data: data,
							...
							function () {
								if (userInfo.isSalesman) {
									...
									if (res.data.syncCymQualification) {
										...
										// console.log('syncSkipData',syncSkipData);
										if (
											!syncSkipData ||
